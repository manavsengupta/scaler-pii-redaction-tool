...
class Redactor:
    def __init__(self):
        ...
        # load global name db (takes a couple secs to load in memory)
                ...
                for c in row.cells:
                    for p in c.paragraphs:
                        if(p.text):
                            p.text=self.proc_txt(p.text)
        doc.save(outp)
        print(f"[+] saved redacted docx to: {outp}")
        self.export_audit()
    ...
    def export_audit(self):
        ...
        print(f"[+] total unique entities processed: {len(df)}")
