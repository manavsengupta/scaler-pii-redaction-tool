...
st.caption("Upload a Word document to detect and anonymize sensitive information.")
    ...
    if st.button("Start Redaction", type="primary"):
        with st.spinner("Processing document..."):
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                output_path = os.path.join(tmp_dir, "redacted_output.docx")
                
                with open(input_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                
                redactor = Redactor()
                redactor.run_docx(input_path, output_path)
                
                with open(output_path, "rb") as f:
                    docx_bytes = f.read()
                
                df_audit = pd.DataFrame(redactor.audit_log)
                if not df_audit.empty:
                    df_audit = df_audit.drop_duplicates(subset=["Original Text", "Entity Type"])
                ...
                if not df_audit.empty:
                    ...
                    m1, m2 = st.columns(2)
                    m1.metric("Entities Sanitized", len(df_audit))
                    m2.metric("Entity Types Found", df_audit["Entity Type"].nunique())
